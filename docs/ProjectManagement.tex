...
\begin{itemize}
	\item Set \textbf{canonical\_proof\_config} to the result of the algorithm described in chapter \ref{subsubsec:proof_config} (maybe also input the public key)
	\item Set \textbf{transformed\_document} to the result of the algorithm described in chapter \ref{chap:transform} with the inputs \textbf{vc} and \textbf{mandatory\_attributes}
	...
	\item Set \textbf{base\_proof} to the result of the algorithm described in chapter \ref{subsubsec:sign} with the inputs \textbf{hash\_data}, \textbf{transformed\_document}, \textbf{hmac\_key} and \textbf{mandatory\_attributes}.
	\item Set \textbf{signed\_vc} to the result of the algorithm described in chapter \ref{subsubsec:signed} with the inputs \textbf{vc}, somehow proof\_config and \textbf{base\_proof}
\end{itemize}
...
The required input of this algorithm is random 32-bit long bit-string named \textbf{hmac\_key} which will be used to initialize a \textit{HMAC}.
...
\begin{itemize}
	...
	\item Return an object containing "mandatory" set to mandatory, "nonMandatory" set to nonMandatory.
\end{itemize}
...
\begin{itemize}
	...
	\item Set cbor to result of the application of the CBOR-encoding\cite{cbor}, where CBOR tagging \textbf{Must NOT} be used. on the array 
